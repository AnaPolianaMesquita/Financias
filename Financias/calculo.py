...
def login():
    ...
    conn = sqlite3.connect('calculo.db')
    cursor = conn.cursor()
...
def salvar_meta():
    meta_nome = request.form['meta_nome']
    meta_valor = request.form['meta_valor']
    # Salvar os dados em um banco de dados ou arquivo
    # Por exemplo, usando um banco de dados SQLite
    import sqlite3
    conn = sqlite3.connect('metas.db')
    c = conn.cursor()
    c.execute("INSERT INTO metas (nome, valor) VALUES (?, ?)", (meta_nome, meta_valor))
    conn.commit()
    conn.close()
    return 'Meta salva com sucesso!'@app.route('/metas')
import sqlite3
